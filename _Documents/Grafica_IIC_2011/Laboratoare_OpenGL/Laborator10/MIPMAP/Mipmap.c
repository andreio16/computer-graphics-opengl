/*  mipmap.c
 *  Programul demonstreazã utilizarea tehnicii mipmap pentru maparea texturilor
 *  Pentru fiecare valoare a parametrului level s-a ales o altã valoare solidã.
 *  În acest fel acelasi patrulater este desenat cu culori diferite pe mãsurã 
 *  ce suprafata sa se îndepãrteazã de punctul de vizualizare
 */
#include "glos.h"
...
//se iniþializeazã cele 6 imagini 
void makeImages(void)
{
    int i, j;
...
{    
	...
	//se specificã textura corespunzãtoare fiecãrui nivel
...
void CALLBACK display(void)
{
	...
	//i se vor aplica texturi diferite pe mãsurã ce se îndepãrteazã
